...
export async function POST(req: NextRequest) {
  ...
  try {
    const { title, date, note, time, endTime } = await req.json()
    if (!title?.trim() || !date) return NextResponse.json({ error: '缺少標題或日期' }, { status: 400 })
    const access = await refreshAccessToken(rt)
    const ev = await insertEvent(access, title.trim(), date, (note ?? '').trim(), (time ?? '').trim(), (endTime ?? '').trim())
...
export async function PATCH(req: NextRequest) {
  ...
  if (!rt) return NextResponse.json({ error: '尚未連結 Google 日曆' }, { status: 400 })
  try {
    const { id, title, date, note, time, endTime } = await req.json()
    if (!id) return NextResponse.json({ error: '缺少 id' }, { status: 400 })
    const access = await refreshAccessToken(rt)
    await patchEvent(access, id, { title, date, note, time, endTime })
